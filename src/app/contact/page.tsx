...
async function getContactContent(): Promise<ContactContent> {
  try {
    ...
    return {
      ...CONTACT_DEFAULTS,
      ...data,
      agents: Array.isArray(data.agents) && data.agents.length ? data.agents : CONTACT_DEFAULTS.agents,
          ...
          <div className="grid lg:grid-cols-[1fr_1.2fr] gap-10 max-w-5xl mx-auto">
            ...
            <div className="space-y-5">
              ...
              {c.agents.map((agent, i) => (
                <div key={`${agent.name}-${i}`} className="bg-white rounded-xl border border-line p-6 shadow-sm">
                  <div className="flex items-start gap-4">
                    {agent.avatarUrl ? (
                      ...
                      <div className={`w-20 h-20 rounded-2xl bg-gradient-to-br ${BADGE_GRADIENT[agent.badgeColor] || BADGE_GRADIENT.green} grid place-items-center text-3xl font-black text-white flex-shrink-0`}>
                        {agent.initial}
                      </div>
                    )}
                        ...
                        <ul className="text-sm text-ink-700 space-y-1 mb-4 list-none p-0 m-0">
                          {agent.bullets.map((b, bi) => (
                            <li key={bi} className="flex items-start gap-1.5">
                              <MaterialIcon name="check" className="!text-base text-brand-green-700 mt-0.5 flex-shrink-0" />
                              <span>{b}</span>
                            </li>
                          ))}
                        </ul>
              ...
              <div className="bg-paper-2 rounded-xl p-5 text-sm text-ink-700 space-y-1">
                ...
                {c.companyInfo.companyName && <p>公司名稱：{c.companyInfo.companyName}</p>}
                {c.companyInfo.businessId && <p>統一編號：{c.companyInfo.businessId}</p>}
                {c.companyInfo.address && <p>公司地址：{c.companyInfo.address}</p>}
                ...
                {c.companyInfo.serviceHours && <p>服務時間：{c.companyInfo.serviceHours}</p>}
                {c.companyInfo.serviceArea && <p>服務範圍：{c.companyInfo.serviceArea}</p>}
              </div>
            </div>
