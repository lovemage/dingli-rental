...
- 圖片上傳自動轉 WebP，存入 Railway Volume / 本機 `public/uploads`
- 首頁輪播圖管理（拖曳排序、調整秒數）
- 修改密碼
...
2. 建立 Web 服務，連接此 GitHub Repo（[github.com/lovemage/dingli-rental](https://github.com/lovemage/dingli-rental)）
3. 環境變數設定：
   ```
   ...
   ```
4. 掛載 Volume → 路徑 `/data/uploads`（持久化儲存上傳圖片）
